...
class CometChatConversationListWithMessages extends React.Component {
  ...
  componentDidMount() {
    console.log(this.context, '1312312345345345');
    ...
    messaging().onNotificationOpenedApp((remoteMessage) => {
      const t = JSON.parse(remoteMessage.data.message);
      this.setState({ notificationMessage: t, notificationToggle: true });
      console.log(
      ...
      {
        t.receiverType === 'user'
          ? this.itemClicked(t.data.entities.sender.entity, t.receiverType)
          : this.itemClicked(t.data.entities.receiver.entity, t.receiverType);
      }
      this.setState({ notificationToggle: false });
      console.log(
        this.state.notificationToggle,
        '###############################',
      );
    ...
    messaging()
      ...
      .then((remoteMessage) => {
        CometChat.getLoggedInUser().then((user) => {
          ...
          const t = JSON.parse(remoteMessage.data.message);
          if (remoteMessage) {
            console.log('Notification caused app to open from quit state:', t);
          }
          this.setState({ notificationMessage: t, notificationToggle: true });
          console.log(
            this.state.notificationToggle,
          ...
          );
          {
            t.receiverType === 'user'
              ? this.itemClicked(t.data.entities.sender.entity, t.receiverType)
              : this.itemClicked(
                  t.data.entities.receiver.entity,
                  t.receiverType,
                );
          }
          this.setState({ notificationToggle: false });
  ...
  audioCall = () => {
    try {
      ...
      if (this.state.notificationToggle === true) {
        if (this.state.type === 'user') {
          receiverId =
            this.state.notificationMessage.data.entities.sender.entity.uid;
      ...
      } else if (this.state.notificationToggle === false) {
        if (this.state.type === 'user') {
  ...
  videoCall = (flag) => {
    try {
      ...
      if (this.state.notificationToggle === false) {
        receiverId = this.state.item.uid;
        receiverType = CometChat.RECEIVER_TYPE.USER;
      } else if (this.state.notificationToggle === true) {
        receiverId =
          this.state.notificationMessage.data.entities.sender.entity.uid;
        receiverType = CometChat.RECEIVER_TYPE.USER;
      }
