...
const StarMessages = ({ navigation }) => {
  ...
  const FetchStarMessages = async () => {
    ...
    await CometChat.callExtension('save-message', 'GET', URL, null)
      .then((response) => {
        setSaved(response);
        console.log(response.savedMessages.length);
        // {savedMessages: []}
  ...
  return (
    <SafeAreaView>
      ...
      <View style={styles.StarMessagesContent}>
        ...
        {saved.savedMessages && loggedInUser != null ? (
          <>
            {saved.savedMessages.length > 0 ? (
              ...
              <View
                ...
                <Text style={{ color: themee.color }}>NO STARRED MESSAGE</Text>
              </View>
            )}
          </>
        ) : (
